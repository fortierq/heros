...
export default function AdventureCard({ adventure, onSelect }: AdventureCardProps) {
  return (
    <button
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        {/* Theme Badge */}
        <div className={`absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium ${themeBadgeColors[adventure.theme]}`}>
      ...
      <div className="p-6">
        ...
        <p className="text-sm text-gold/70 font-medium mb-3">{adventure.subtitle}</p>
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center group-hover:bg-gold/20 transition-colors">
            <span className="text-lg">{adventure.theme === 'fantasy' ? '⚔️' : adventure.theme === 'scifi' ? '🚀' : '⚡'}</span>
          </div>
        </div>
      </div>
    </button>
