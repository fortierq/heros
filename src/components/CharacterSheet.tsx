...
import { Heart, Sword, Shield, Sparkles, Clover, Star } from 'lucide-react';
...
export default function CharacterSheet() {
  ...
  const hpColor = hpPercent > 60 ? 'hp-bar-high' : hpPercent > 30 ? 'hp-bar-mid' : 'hp-bar-low';
    ...
    <div className="bg-gray-900/80 border border-gray-700/50 rounded-xl p-4 space-y-3">
      ...
      <div>
        <div className="flex items-center justify-between text-xs mb-1">
          ...
          <span className="text-gray-400">{stats.hp}/{stats.maxHp}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        <StatItem icon={<Sword className="w-3.5 h-3.5 text-orange-400" />} label="Attaque" value={stats.attack} />
        <StatItem icon={<Shield className="w-3.5 h-3.5 text-blue-400" />} label="Défense" value={stats.defense} />
        <StatItem icon={<Sparkles className="w-3.5 h-3.5 text-purple-400" />} label="Magie" value={stats.magic} />
        <StatItem icon={<Clover className="w-3.5 h-3.5 text-green-400" />} label="Chance" value={stats.luck} />
      </div>
    </div>
  );
}
